...
export const TerminalWindow: React.FC<TerminalProps> = ({ children }) => {
  ...
  return (
    <motion.div
      ...
      className="relative bg-cyber-darker/90 rounded-lg p-4 font-mono text-cyber-neon"
    >
      {/* Terminal header */}
      <div className="flex items-center justify-between mb-4 border-b border-cyber-neon/20 pb-2">
        <div className="flex space-x-2">
          <div 
            className={`w-3 h-3 rounded-full ${isActive ? 'bg-cyber-neon' : 'bg-cyber-pink'}`}
      ...
      <div className="space-y-2">
        {messages.map((msg, idx) => (
          <div key={idx} className="text-cyber-neon/70">{msg}</div>
        ))}
        <div className={`transition-opacity duration-200 ${isActive ? 'opacity-100' : 'opacity-50'}`}>
          {children}
        </div>
      </div>
